...
SCENARIO("libkameris sparse distances (normal)", "[libkameris][distances-sparse]") {
	GIVEN("Two random sparse vectors where the second is longer than the first") {
		WHEN("using as kameris::SparseVectorAdapters") {
...
SCENARIO("libkameris sparse distances (exceptions)", "[libkameris][distances-sparse]") {
